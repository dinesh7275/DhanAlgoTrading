...
class LiveRiskManager:
    ...
    def __init__(self, initial_capital, max_daily_loss=0.10, max_portfolio_loss=0.15):
        self.initial_capital = initial_capital
        self.current_capital = initial_capital
        
        # Updated risk limits for options trading
        self.max_daily_loss = max_daily_loss  # 10% max daily loss
        self.max_portfolio_loss = max_portfolio_loss  # 15% max portfolio loss
        self.max_position_risk = 0.10  # 10% risk per trade
        self.max_trades_per_day = 10   # Maximum 10 trades per day
        self.daily_profit_target = 0.10  # 10% daily profit target
        self.max_correlation_exposure = 0.5  # Higher for options
        self.max_single_position = 0.33  # Max 33% per position for options
    ...
    def reset_daily_counters(self):
        """Reset daily tracking counters at market open"""
    ...
    def can_place_trade(self):
        """Check if new trade can be placed based on daily limits"""
        ...
        if self.target_achieved:
            return False, "Daily profit target achieved - stopping trading for the day"
        ...
        if daily_return <= -self.max_daily_loss:
            return False, f"Daily loss limit reached ({self.max_daily_loss*100:.1f}%)"
    ...
    def update_trade_result(self, pnl):
        ...
        if daily_return >= self.daily_profit_target:
            ...
            self._trigger_alert('INFO', f'Daily profit target achieved: {daily_return*100:.2f}%')
        
        print(f"Trade #{self.trades_today} completed. Daily P&L: Rs.{self.daily_pnl:.2f} ({daily_return*100:.2f}%)")
    
    def calculate_indian_taxes(self, premium, quantity, pnl):
        """Calculate Indian taxes for options trading"""
        if pnl <= 0:
            return 0
        
        # STT on options (0.017% on sell side premium)
        stt = premium * quantity * 0.00017
        
        # Exchange transaction charges (approx 0.0019%)
        exchange_charges = premium * quantity * 0.000019
    ...
    def calculate_net_pnl(self, gross_pnl, premium, quantity):
        """Calculate net P&L after Indian taxes"""
        taxes = self.calculate_indian_taxes(premium, quantity, gross_pnl)
